                        ...
                        <div class="container-fluid">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="card">
                                        <div class="header">
                                            <h4 class="title">Grant users Admin Role</h4>
                                            <p class="category"></p>
                                        </div>
...
//$print = isset($LastTimeLogin[$i]) ? $LastTimeLogin[$i] : "--"
                                                    ...
                                                    for ($i = 0; $i < $size; $i++)
                                                    {
                                                        ...
                                                        echo "<td><strong id='Sub$rowID[$i]'>" . $Timestamp_Authorizes = $timstAdminApproval[$i] == 0 ? " Pending " : date("d/m/Y h:i:s", $timstAdminApproval[$i]) . "</strong> </td>";
                                                        echo "<td> --- </td>";
                                                        $button = $timstAdminApproval[$i] == 0 ? "<button type='submit' class='btn btn-info btn-fill Upload$rowID[$i] uploadID orange'  data-title='$groupname' data-id='$rowID[$i]' data-status='2' style=' margin-left:-65px;width:115px;'>Pending</button>" : "<button type='submit' class='btn btn-info btn-fill Upload$rowID[$i] uploadID'  data-title='$groupname' data-id='$rowID[$i]' data-status='1' style=' margin-left:-65px;'>On AIR </button>"; //background-color:red;
                                                        ...
                                                        echo "<td><button type='submit' class='btn btn-info btn-fill remove$rowID[$i] removeID red darken-2' id='removeID' data-id='$rowID[$i]' style=' margin-left:-65px;background-color:green;'>Delete</button> </td>";
...
<?php $this->view('extra/jslib'); ?>
            ...
            $.ajax({
                ...
                data: {
                    ...
                    action: "admin grant user lead group",
        ...
        $('.removeID').click(function () {
            ...
            $.ajax({
                type: "POST",
                url: "<? echo site_url('Cocialnetwork/reprod'); ?>",
                dataType: 'json',
                cache: false,
                data: {
                    type_of_action: "click",
        ...
        $('.uploadID').click(function () {
            ...
            $.ajax({
                type: "POST",
                url: "<? echo site_url('Cocialnetwork/upprod'); ?>",
                dataType: 'json',
                cache: false,
                data: {
                    type_of_action: "click",
